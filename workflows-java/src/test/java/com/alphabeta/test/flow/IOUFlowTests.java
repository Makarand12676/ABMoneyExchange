package com.alphabeta.test.flow;
...
public class IOUFlowTests {
    ...
    public void setup() {
        network = new MockNetwork(new MockNetworkParameters().withCordappsForAllNodes(ImmutableList.of(
                TestCordapp.findCordapp("com.alphabeta.contract"),
                TestCordapp.findCordapp("com.alphabeta.flow"))));
        a = network.createPartyNode(null);
        b = network.createPartyNode(null);
        // For real nodes this happens automatically, but we have to manually register the flow for tests.
        for (StartedMockNode node : ImmutableList.of(a, b)) {
            node.registerInitiatedFlow(alphabetaFlow.Acceptor.class);
        }
        network.runNetwork();
    }
    ...
    public void flowRejectsInvalidIOUs() throws Exception {
        ...
        alphabetaFlow.Initiator flow = new alphabetaFlow.Initiator(-1, b.getInfo().getLegalIdentities().get(0));
    ...
    public void recordedTransactionHasNoInputsAndASingleOutputTheInputIOU() throws Exception {
        Integer iouValue = 1;
        alphabetaFlow.Initiator flow = new alphabetaFlow.Initiator(iouValue, b.getInfo().getLegalIdentities().get(0));
        CordaFuture<SignedTransaction> future = a.startFlow(flow);
        network.runNetwork();
        SignedTransaction signedTx = future.get();
