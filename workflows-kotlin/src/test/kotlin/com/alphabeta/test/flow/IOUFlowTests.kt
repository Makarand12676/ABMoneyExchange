...
class IOUFlowTests {
    ...
    fun setup() {
        ...
        listOf(a, b).forEach { it.registerInitiatedFlow(alphabetaFlow.Acceptor::class.java) }
        network.runNetwork()
    }
    ...
    fun `flow rejects invalid IOUs`() {
        val flow = alphabetaFlow.Initiator(-1, b.info.singleIdentity())
    ...
    fun `recorded transaction has no inputs and a single output, the input IOU`() {
        val iouValue = 1
        val flow = alphabetaFlow.Initiator(iouValue, b.info.singleIdentity())
        val future = a.startFlow(flow)
        network.runNetwork()
        val signedTx = future.getOrThrow()
